...
def _serialize_member_roles(
    current_role: str | None, member_roles: list[enterprise_rbac_service.MemberRoleSummary]
) -> list[dict[str, str]]:
    if member_roles:
...
class MemberInviteEmailApi(Resource):
    ...
    def post(self):
        ...
        if not dify_config.RBAC_ENABLED and (
            not TenantAccountRole.is_valid_role(invitee_role) or not TenantAccountRole.is_non_owner_role(invitee_role)
        ):
